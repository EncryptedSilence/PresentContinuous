...
def compose(emissions, meta, out_path):
    ...
    a("#")
    a("#   Stated honestly, second part: the relink experiment was run on a 39-row")
    a("#   image, before the ten bitslice64 rows existed. It has NOT been re-measured")
    a("#   on the image that produced this file, and by the argument three")
    a("#   paragraphs up -- a firmware change recompiles the objects, so the 7.5%")
    a("#   does not bound the difference across one -- carrying it here is an")
    a("#   assumption, not a measurement. The mechanism (offset mod 16 against a")
    a("#   128-bit flash word) is a property of the part and does not depend on the")
    a("#   row set, so the figure is expected to be about right; it is quoted as the")
    a("#   working floor rather than as a value measured on this image. Anything")
    a("#   that turns on the third significant figure of a cross-config ratio should")
    a("#   re-run the relink on the current objects first.")
    a("#")
    ...
    a("#")
    a("#   One exception is relayed from the firmware below and is flagged here so it")
    a("#   is not read as an endorsed figure: the `tables:` note quotes per-row")
    a("#   percentages (+6.99% / +6.73%) for the T-table CCM-vs-SRAM A/B. Those are")
    a("#   single-row differences between two separate builds -- the case this")
    a("#   paragraph forbids, and worse than a relink, since a different table")
    a("#   placement recompiles. They are quoted for their SIGN, which is the whole")
    a("#   of the conclusion (CCM is slower for those tables); the magnitude is")
    a("#   inside the floor and means nothing.")
    a("#")
