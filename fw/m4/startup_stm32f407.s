...
3:  cmp     r0, r1
    ...
    /* .ramtext <- its load image. The sram-noart configuration runs the cipher
     * and the harness from SRAM; product.ld defines the range empty, so this
     * loop exits immediately for the product and flash-noart builds and all
     * three boot the same way. The load address comes from the linker
     * (_sramtext_load = LOADADDR(.ramtext)) rather than being assumed to follow
     * .text.
     *
