...
`timescale 1ns/1ps
module tb_present_80_lin444_297_r7_area;
reg clk = 0, rst = 1, start = 0;
reg [63:0] plaintext;
reg [79:0] key;
wire [63:0] ciphertext;
wire valid, busy;
present_80_lin444_297_r7_area dut(.clk(clk), .rst(rst), .start(start), .plaintext(plaintext), .key(key), .ciphertext(ciphertext), .valid(valid), .busy(busy));
always #5 clk = ~clk;
integer errors = 0;
task run_vec;
  input [79:0] k;
...
initial begin
  ...
  $display("PASS present_80_lin444_297_r7_area");
  $finish;
end
endmodule
