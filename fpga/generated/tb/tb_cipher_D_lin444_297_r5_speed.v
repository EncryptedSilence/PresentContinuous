...
`timescale 1ns/1ps
module tb_cipher_D_lin444_297_r5_speed;
reg clk = 0, rst = 1, start = 0;
reg [63:0] plaintext;
reg [383:0] key;
wire [63:0] ciphertext;
wire valid, busy;
cipher_D_lin444_297_r5_speed dut(.clk(clk), .rst(rst), .start(start), .plaintext(plaintext), .key(key), .ciphertext(ciphertext), .valid(valid), .busy(busy));
always #5 clk = ~clk;
integer errors = 0;
task run_vec;
  input [383:0] k;
