...
`timescale 1ns/1ps
module cipher_D_speed_gowin_top(
  input wire clk,
  input wire rst,
...
wire valid;
wire busy;
cipher_D_speed core(.clk(clk), .rst(rst), .start(start), .plaintext(plaintext), .key(key), .ciphertext(ciphertext), .valid(valid), .busy(busy));
always @(posedge clk) begin
  if (rst) begin
    ...
    if (!busy) begin
      ...
      key <= {key[511:0], key[575:512] ^ 64'h9e3779b97f4a7c15};
    end
