...
`timescale 1ns/1ps
module cipher_D_lin444_297_r5_speed_gowin_top(
  input wire clk,
  input wire rst,
...
wire valid;
wire busy;
cipher_D_lin444_297_r5_speed core(.clk(clk), .rst(rst), .start(start), .plaintext(plaintext), .key(key), .ciphertext(ciphertext), .valid(valid), .busy(busy));
