...
`timescale 1ns/1ps
module present_80_lin444_297_r7_speed_gowin_top(
  input wire clk,
  input wire rst,
...
wire valid;
wire busy;
present_80_lin444_297_r7_speed core(.clk(clk), .rst(rst), .start(start), .plaintext(plaintext), .key(key), .ciphertext(ciphertext), .valid(valid), .busy(busy));
always @(posedge clk) begin
  if (rst) begin
    ...
    if (!busy) begin
      ...
      key <= {key[15:0], key[79:16] ^ 64'h9e3779b97f4a7c15};
    end
