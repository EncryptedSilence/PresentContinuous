...
`timescale 1ns/1ps
module present_80_lin444_297_r7_area_gowin_top(
  input wire clk,
  input wire rst,
...
wire valid;
wire busy;
present_80_lin444_297_r7_area core(.clk(clk), .rst(rst), .start(start), .plaintext(plaintext), .key(key), .ciphertext(ciphertext), .valid(valid), .busy(busy));
